...
-- 14. Identify the day with the highest total revenue.
...
-- 15. Find the average order value (AOV).
SELECT ROUND(SUM(order_details.quantity * pizzas.price) / COUNT(DISTINCT orders.order_id), 2) AS avg_order_value
FROM order_details
JOIN pizzas ON order_details.pizza_id = pizzas.pizza_id
JOIN orders ON orders.order_id = order_details.order_id;

-- 16. Find the total number of pizzas sold by category and size.
SELECT pizza_types.category, pizzas.size, SUM(order_details.quantity) AS total_sold
FROM order_details
JOIN pizzas ON order_details.pizza_id = pizzas.pizza_id
